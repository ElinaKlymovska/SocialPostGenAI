...
from chat.telegram.start_telegram_bot import start_telegram_bot
...
def display_telegram_integration():
    """
    Відображає статус Telegram-бота та запускає його через Streamlit.
    """
    st.sidebar.header("Telegram Integration")

    if TELEGRAM_TOKEN:
        if st.sidebar.button("Start Telegram Bot"):
            st.sidebar.success("Starting Telegram Bot...")
            # Запускаємо бота в окремому потоці
            thread = threading.Thread(target=start_telegram_bot, args=(TELEGRAM_TOKEN,))
            thread.daemon = True
            thread.start()
            st.sidebar.info("Telegram Bot is running. Check your Telegram.")
    else:
        st.sidebar.error("Telegram token not found. Please set it in the .env file.")
